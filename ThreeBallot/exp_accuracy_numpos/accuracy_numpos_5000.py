...
if   gen_dataset == 0:
    nu = 100000 #Users
    ratio = 0.9 #Fraction with all traits
    lst_traits = [0,1] #Traits
    num_traits = len(lst_traits)
    nsplits = nu * num_shares

    #Generate PseudoRandom Dataset (total_size-ratio*total_size)
    negative_nu = math.floor(nu - (nu * ratio))
    positive_nu = math.ceil(nu * ratio)

    ds_neg = pseudorandom_dataset(negative_nu, lst_traits)
    ds_pos = dataset_with_trait(negative_nu, positive_nu, lst_traits)

    ds = dict(ds_neg)
    ds.update(ds_pos)
elif gen_dataset == 1: #Load T10I4D100K or any other generated dataset
    #raw_data = open('datasets/T10I4D100K.txt')
    #raw_data = open('datasets/IBM_Quest_data_generator/data.data')
    #raw_data = open('datasets/bakery/1200000_synth.txt')
    raw_data = open('datasets/synthetic/0.25_tsz7_tct1.0m.txt')
    ...
    #print(ds)
elif gen_dataset == 2:  # Load basket dataset
    raw_data = open('datasets/basket.txt')
    ...
    items = list(items)
    '''
    lst_traits = list(['heineken', 'chicken'])
    items = lst_traits
    print(items)
    ...
    # print(ds)
elif gen_dataset == 3: #Load KASANDR
    raw_data = open('C:\datasets\\KASANDR_user_test.csv')
    ...
    items = list(items)
    '''
    lst_traits = list(['90260c5ee5ff34595171760e6376fb15', 'a4a17c8c0ba213a50c48fd4251194fe2'])
    items = lst_traits
    print(items)
...
for iter in range(100): #Experiment repeat
    ...
    with open("results//num_of_pos//" + '-'.join(lst_traits)+".csv", "a+") as results:
        print(','.join([str(counts_original[-1]), str(iter), str(support_err), str(confidence_err), str(lift_err)]))
        results.write(','.join([str(counts_original[-1]), str(iter), str(support_err), str(confidence_err), str(lift_err)])+"\n")
